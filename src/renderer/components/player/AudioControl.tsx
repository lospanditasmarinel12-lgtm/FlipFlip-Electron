...
class AudioControl extends React.Component {
    ...
    return (
      <React.Fragment key={audio.id}>
        ...
        {this.props.audioEnabled && this.props.audio.tick && this.state.playing && (
          <SoundTick
            url={toFileUrl(this.props.audio.url)}
            playing={playing}
  ...
  onPlaying(soundData: any) {
    ...
    const audioEl = soundData?._a;
    if (audioEl && audioEl instanceof HTMLAudioElement && audioEl !== this._audioElement) {
      this._audioElement = audioEl;
      // Record the element even if no toy is connected yet — startAnalysis
      // stores it and defers, so a toy connected after playback can auto-start
      // scene-audio analysis (see HapticService.reconnect).
      HapticService.getInstance().startAnalysis(audioEl);
    }
  }
