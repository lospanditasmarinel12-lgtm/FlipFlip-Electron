...
export default class Player extends React.Component {
  ...
  readonly state = {
    ...
    hideCursor: false,
    hapticDevices: [] as Array<{ index: number; name: string }>,
    hapticActive: false,
  };
  ...
  render() {
    ...
    if (this.props.config.generalSettings.watermark && (!this.props.gridView || this.props.config.generalSettings.watermarkGrid)) {
      ...
      if (img) {
        watermarkText = watermarkText.replace("{source_url}", img.source);
        watermarkText = watermarkText.replace("{source_name}", getFileGroup(img.source));
        if (img.post) {
          watermarkText = watermarkText.replace("{post_url}", img.post);
        } else {
          watermarkText = watermarkText.replace(/\{post_url\}\s*/g, "");
        }
        watermarkText = watermarkText.replace("{file_url}", img.url.startsWith("file") ? urlToPath(img.url) : img.url);
        watermarkText = watermarkText.replace("{file_name}", decodeURIComponent(getFileName(img.url)));
      } else {
      ...
      <div style={rootStyle}>
        ...
        {!this.props.gridView && (
          <PlayerBars
            ...
            hapticDevices={this.state.hapticDevices}
            hapticActive={this.state.hapticActive}
            ...
            onHapticsEnabledChange={this._onHapticsEnabledChange}
  ...
  componentDidMount() {
    ...
    if (!this._hapticsInitialized && this.props.scene.hapticsEnabled) {
      this._hapticsInitialized = true;
      HapticService.getInstance().setConfig({
        enabled: true,
        intensity: this.props.scene.hapticIntensity,
        pattern: this.props.scene.hapticPattern as any,
      });
      HapticService.getInstance().setActiveDevice(this.props.scene.hapticActiveDevice);
      HapticService.getInstance().init(
        (devices) => this.setState({ hapticDevices: devices }),
        (active) => this.setState({ hapticActive: active }),
        this.props.config.generalSettings.hapticTransport,
        this.props.config.generalSettings.hapticWSEndpoint,
        this.props.config.generalSettings.hapticSystemAudio,
        this.props.config.generalSettings.hapticSystemDeviceId
      ).then(() => {
        HapticService.getInstance().setConfig({
          enabled: this.props.scene.hapticsEnabled,
          intensity: this.props.scene.hapticIntensity,
          pattern: this.props.scene.hapticPattern as any,
        });
        HapticService.getInstance().setActiveDevice(this.props.scene.hapticActiveDevice);
  ...
  componentWillUnmount() {
    ...
    // Defer cache teardown so leaving playback doesn't block the main thread.
    // (Global resource-cache clearing is the heavy hammer; Chromium frees
    // decoded image memory via Oilpan GC once eviction drops element clients.)
    ...
    if (this._hapticsInitialized) {
      this._hapticsInitialized = false;
      HapticService.getInstance().shutdown();
    }
  }

  _onHapticsEnabledChange = (enabled: boolean) => {
    if (enabled) {
      this._hapticsInitialized = true;
      HapticService.getInstance().setConfig({
        enabled: true,
      ...
      HapticService.getInstance().init(
        ...
        HapticService.getInstance().tryStartPendingAnalysis();
      });
    } else {
      this._hapticsInitialized = false;
      HapticService.getInstance().shutdown();
  ...
  play() {
    this.setState((state: any) => {
      const next: any = {isPlaying: true};
      // Resuming after stepping Back: jump back to the live playhead so
      // auto-advance can resume (started is already true, so LiveShowPlayer
      // re-arms its dwell timer when it sees isPlaying true + offset 0).
      if (state.historyOffset != null && state.historyOffset < 0) {
        next.historyOffset = 0;
      }
      return next;
    });
