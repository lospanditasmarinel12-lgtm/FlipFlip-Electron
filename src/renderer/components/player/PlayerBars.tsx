...
const AppBarStyled = styled(AppBar, { shouldForwardProp: (prop) => prop !== 'hovered' })<{ hovered?: boolean }>(
  ({ theme, hovered }) => ({
    ...
    marginTop: hovered ? 0 : theme.spacing(-8.5),
...
const DrawerStyled = styled(Drawer, { shouldForwardProp: (prop) => prop !== 'hovered' })<{ hovered?: boolean }>(
  ({ theme, hovered }) => ({
    zIndex: theme.zIndex.drawer,
    width: drawerWidth,
    marginLeft: hovered ? 0 : -drawerWidth - 3,
    transition: theme.transitions.create('margin', {
      easing: theme.transitions.easing.sharp,
      duration: hovered
        ? theme.transitions.duration.enteringScreen
...
class PlayerBars extends React.Component {
  ...
  _drawerTouchStartX: number | null = null;
  _drawerTouchStartY: number | null = null;
        ...
        <AppBarStyled
          ...
          <HeaderBar>
            ...
            <Tooltip disableInteractive title={this.props.title}>
              <Typography component="h1" variant="h4" color="inherit" noWrap sx={{ textAlign: 'center', flexGrow: 1 }}>
                {this.props.title}
              </Typography>
            </Tooltip>

            <div style={{ flexBasis: '20%', justifyContent: 'flex-end', display: 'flex' }}>
        ...
        {this.props.hasStarted && !this.props.isEmpty && !this.props.recentPictureGrid && !this.props.scene.downloadScene && (
          <React.Fragment>
            ...
            <DrawerStyled
              ...
              onMouseEnter={this.onMouseEnterDrawer.bind(this)}
              onMouseLeave={this.onMouseLeaveDrawer.bind(this)}
              onTouchStart={this.onDrawerTouchStart.bind(this)}
              onTouchMove={this.onDrawerTouchMove.bind(this)}>
              <DrawerToolbar>
                ...
                <Accordion defaultExpanded={this.props.scene.audioScene} TransitionProps={{ unmountOnExit: !this.props.scene.audioEnabled && this.props.scene.nextSceneID === 0 }}>
                  ...
                  <AccordionDetails>
                    <AudioCard
                      sidebar
                      scene={this.props.scene}
                      scenePaths={this.props.scene.audioPlaylists}
                      startPlaying
  ...
  closeDrawer() {
    ...
    this._drawerTouchStartX = null;
    this._drawerTouchStartY = null;
  ...
  // Touch (touchscreens/tablets): open the drawer and keep it open — close via
  // the edge strip or a swipe-down gesture, matching the mobile app.
  onTouchDrawerToggle() {
  ...
  onDrawerTouchStart(e: any) {
    ...
    if (!t) return;
    this._drawerTouchStartX = t.clientX;
    this._drawerTouchStartY = t.clientY;
  }
  ...
  onDrawerTouchMove(e: any) {
    ...
    const t = e.touches && e.touches[0];
    if (!t) return;
    if (this._drawerTouchStartX == null || this._drawerTouchStartY == null) return;
    const dx = t.clientX - this._drawerTouchStartX;
    const dy = t.clientY - this._drawerTouchStartY;
  ...
  setMenuBarVisibility(showMenu: boolean) {
    (this.props.config.displaySettings as any).showMenu = showMenu;
    this.buildMenu();
    getCurrentWindow().setMenuBarVisibility(showMenu);
  }

  setFullscreen(fullScreen: boolean) {
    (this.props.config.displaySettings as any).fullScreen = fullScreen;
    this.buildMenu();
  ...
  showContextMenu = (e: MouseEvent) => {
    ...
    const rec = this.props.historyPaths[(this.props.historyPaths.length - 1) + this.props.historyOffset];
    const url = rec ? rec.url : "";
    let source = rec ? rec.source : "";
    let post = rec ? rec.post : null;
  ...
  setAlwaysOnTop(alwaysOnTop: boolean){
    (this.props.config.displaySettings as any).alwaysOnTop = alwaysOnTop;
    this.buildMenu();
