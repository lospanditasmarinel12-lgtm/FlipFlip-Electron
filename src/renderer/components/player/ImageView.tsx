...
export default class ImageView extends React.Component {
  ...
  componentDidMount() {
    ...
    this._applyGen = 0;
    this._zeroGridCanvases();
    this._applyImage();
    // Measure the real container size now that the ref is mounted so the first
    // pan frame uses actual dimensions instead of the window fallback.
    if (this.props.scene && this.props.scene.panning) {
      this.forceUpdate();
    }
  }

  _zeroGridCanvases() {
    if (!this.props.gridCoordinates) return;
    const canvasClasses = ['canvas', 'canvas-bg'];
    for (const cls of canvasClasses) {
      const canvases = document.getElementsByClassName(cls + '-' + this.props.gridCoordinates[0] + '-' + this.props.gridCoordinates[1]);
  ...
  _releaseGridCanvases() {
    if (!this.props.gridCoordinates) return;
    const canvasClasses = ['canvas', 'canvas-bg', 'copy', 'copy-bg'];
    for (const cls of canvasClasses) {
      const canvases = document.getElementsByClassName(cls + '-' + this.props.gridCoordinates[0] + '-' + this.props.gridCoordinates[1]);
      for (let i = 0; i < canvases.length; i++) {
        const c = canvases[i] as HTMLCanvasElement;
        ...
        if (c.parentElement) {
          try { c.parentElement.removeChild(c); } catch (e) {}
        }
      }
    }
    const gridKey = this.props.gridCoordinates[0] + '-' + this.props.gridCoordinates[1];
    const clones = document.querySelectorAll(`[data-grid-clone="${gridKey}"]`);
    clones.forEach(el => { try { el.remove(); } catch (e) {} });
  }

  _getOrCreateCanvas(className: string, width: number, height: number): HTMLCanvasElement {
    const existing = document.getElementsByClassName(className);
    if (existing.length > 0) {
      const canvas = existing[0] as HTMLCanvasElement;
  ...
  /**
   * Append a canvas copy of the current media into a grid cell. Using a canvas
   * (vs cloneNode) avoids re-decoding the image/video per mirrored cell, keeping
   * grid playback from multiplying decoded media + GPU textures by the cell
   * count.
   */
  _appendGridCanvas(el: HTMLElement, type: string, img: any, scale: number) {
    if (!this.props.pictureGrid) {
      // Single (non-grid) display: just attach the media element.
      if (el) el.appendChild(img);
      return;
    }
    // Only grid-mirror cells (which share the SAME live media element across
    // many cells) use a canvas copy. The filmstrip (PictureGrid) has one
    // distinct element per cell, so a light clone is fine there and avoids the
    // shared-canvas collision.
    if (!this.props.gridCoordinates) {
      const clone = img.cloneNode() as HTMLElement;
      if (el) el.appendChild(clone);
      return;
    }
    const key = this.props.gridCoordinates[0] + '-' + this.props.gridCoordinates[1];
    if (img instanceof HTMLVideoElement) {
      const cn = "canvas-" + key;
      const canvas = this._getOrCreateCanvas(cn, Math.max(1, img.videoWidth * scale), Math.max(1, img.videoHeight * scale));
      ...
      const cn = "canvas-" + key;
      const canvas = this._getOrCreateCanvas(cn, Math.max(1, img.width * scale), Math.max(1, img.height * scale));
      canvas.style.marginTop = img.style.marginTop;
      canvas.style.marginLeft = img.style.marginLeft;
      canvas.style.transform = img.style.transform;
      canvas.style.transformOrigin = img.style.transformOrigin;
  ...
  componentDidUpdate(props: any) {
    ...
    } else if (props.image !== this.props.image || forceBG) {
      this._applyRAF = requestAnimationFrame(() => this._applyImage(forceBG));
    } else {
      this._applyImage(forceBG);
    }
  ...
  _applyImage(forceBG = false) {
    ...
    const _timedPlay = (v: HTMLVideoElement) => {
      const adv = (v as any).__ff_advanceAt;
      const ready = (v as any).__ff_readyAt;
      if (typeof adv === 'number' && typeof ready === 'number') {
        ...
        if (p && p.then) {
          p.then(() => {
            ...
            console.log(
              '[VideoTransition]',
              `seq=${(v as any).__ff_seq || '?'}`,
              `readyAt=${Math.round(ready)}`, `advanceAt=${Math.round(adv)}`,
              `ready→advance=${Math.round(adv - ready)}`,
              `advance→play=${Math.round(playStart - adv)}`,
              `play=${Math.round(playEnd - playStart)}`
            );
          }).catch(() => {});
          return;
        }
    ...
    if (!forceBG && firstChild && firstChild.src == img.src &&
      ...
      // Same image still on screen — leave any running loops alone.
      return;
    }

    // A different image (or forced bg) is being applied: invalidate any loops
    // from the previous image so stale videos/canvases can't keep running (and
    // holding their decoded frames / GPU textures) forever.
    this._applyGen++;
    this.clearTimeouts();
    ...
    const currentGen = this._applyGen;
    const isStale = () => this._applyGen !== currentGen || this._timeouts == null;
    const _el = el;

    const videoLoop = (v: any) => {
      if (!_el || !_el.parentElement || parseFloat(_el.parentElement.style.opacity) == 0.99 || v.paused || this._timeouts == null) return;
      if (isStale()) return;
      if (v.ended) {
        v.onended(null);
        return;
      ...
      if (!this.props.pictureGrid && this.props.hasStarted && this.props.scene.crossFade && crossFadeAudio && v instanceof HTMLVideoElement) {
        ...
        const parentVolume = parseFloat(_el.parentElement.parentElement.getAttribute("volume"));
        if (!isNaN(parentVolume)) {
          v.volume = (volume / 100) * parentVolume;
        }
      }
    ...
    const drawLoop = (v: any, c: CanvasRenderingContext2D, w: number, h: number) => {
      if (!_el || !_el.parentElement || parseFloat(_el.parentElement.style.opacity) == 0.99 || this._timeouts == null) return;
      if (isStale()) return;
    ...
    const extraDrawLoop = (v: any, w: number, h: number) => {
      if (!_el || !_el.parentElement || parseFloat(_el.parentElement.style.opacity) == 0.99 || v.ended || v.paused || this._timeouts == null) return;
      if (isStale()) return;
    ...
    if (img instanceof HTMLImageElement) {
      ...
      if (img.paused) _timedPlay(img)
    } else if (img instanceof HTMLIFrameElement) {
      type = ST.nimja;
    }
    if (type != ST.nimja && (!imgWidth || !imgHeight)) return;
    ...
    if (!forceBG) {
      ...
      const  appendOriginal = () => {
        ...
        // Grid cells: use a lightweight canvas copy instead of cloneNode()
        // (a cloned <video>/<img> would re-decode and double the decoded media +
        // GPU textures for every mirrored cell).
        this._appendGridCanvas(el, type, img, scale);
      }
      if (this.props.gridCoordinates) {
        for (let element of document.getElementsByClassName("copy-" + this.props.gridCoordinates[0] + "-" + this.props.gridCoordinates[1])) {
          if (element == el) {
            ...
            if (img instanceof HTMLVideoElement) {
              const cn = "canvas-" + this.props.gridCoordinates[0] + "-" + this.props.gridCoordinates[1];
              const canvas = this._getOrCreateCanvas(cn, img.videoWidth * scale, img.videoHeight * scale);
              canvas.style.marginTop = img.style.marginTop;
              canvas.style.marginLeft = img.style.marginLeft;
              canvas.style.transform = img.style.transform;
              canvas.style.transformOrigin = img.style.transformOrigin;
              ...
              this._appendGridCanvas(element as HTMLElement, type, img, scale);
            }
          }
        }
      } else {
        ...
        for (let element of document.getElementsByClassName("copy-bg-" + this.props.gridCoordinates[0] + "-" + this.props.gridCoordinates[1])) {
          if (element == bg) {
            ...
            if (img instanceof HTMLVideoElement || bgImg instanceof HTMLCanvasElement) {
              const cn = "canvas-bg-" + this.props.gridCoordinates[0] + "-" + this.props.gridCoordinates[1];
              const canvas = this._getOrCreateCanvas(cn, bgImg.width, bgImg.height);
              canvas.style.width = bgImg.style.width;
              canvas.style.height = bgImg.height;
      ...
      for (let element of document.getElementsByClassName("copy-" + this.props.gridCoordinates[0] + "-" + this.props.gridCoordinates[1])) {
        if (el && img && img.src == this.props.image.src) {
          if (element == el) {
            ...
            // Canvas copy, never cloneNode(): avoids re-decoding per cell.
            this._appendGridCanvas(element as HTMLElement, null, img, scale);
          }
        }
      }
    } else {
