  ...
  render() {
    ...
    return (
      <Grid container spacing={1}>
        {grid.map((c, x) =>
          <Grid key={x} item xs={12} sm={6} md={4} lg={3} sx={theme => ({
            ...((this.props.tutorial == SDGT.edit1 || this.props.tutorial == SDGT.edit2) && { zIndex: `${theme.zIndex.modal + 1} !important` as any }),
          })}>
            <Grid container spacing={1}>
              {c.map((wg: WeightGroup, y) =>
                <Grid key={y} xs={12} item>
                  <Card>
                    <CardHeader
                      slotProps={{ action: { sx: { alignSelf: 'unset' } } }}
                      avatar={
                        <IconButton
                          sx={theme => ({
                            padding: 0,
                            fontSize: '1.125rem',
                            ...(this.props.tutorial == SDGT.edit1 && { borderWidth: 2, borderColor: theme.palette.secondary.main, borderStyle: 'solid' }),
                          })}
                          onClick={this.onWeighGroup.bind(this, (y*4)+x)}
                          size="large">
                          {wg.type == TT.weight && (
                            <Avatar sx={theme => ({
                              backgroundColor: (wg.rules && !this.areRulesValid(wg)) ? theme.palette.error.main : theme.palette.primary.light,
                              color: (wg.rules && !this.areRulesValid(wg)) ? theme.palette.error.contrastText : theme.palette.primary.contrastText,
                            })}>
                              {wg.percent}
                            </Avatar>
                          )}
                      ...
                      action={
                        <React.Fragment>
                          ...
                          <IconButton size="small"
                                      sx={(this.props.tutorial == SDGT.edit1 || this.props.tutorial == SDGT.edit2) ? { pointerEvents: 'none' } : undefined}
        ...
        <Menu
          ...
          keepMounted
          sx={theme => ({
            ...((this.state.isEditing != -1 || this.props.tutorial == SDGT.edit2) && { zIndex: `${theme.zIndex.modal + 1} !important` as any }),
          })}
          slotProps={{
            list: { sx: { display: 'flex', padding: 1, overflow: 'hidden' } },
            paper: { sx: theme => ({
              ...(this.state.isEditing != -1 && { zIndex: `${theme.zIndex.modal + 1} !important` as any }),
              ...(this.props.tutorial == SDGT.edit2 && { borderWidth: 2, borderColor: theme.palette.secondary.main, borderStyle: 'solid' }),
            }) },
          }}
          ...
          {this.state.isWeighing != -1 && (
            <Box sx={theme => ({
              height: 'auto',
              transform: isWeighing.type != TT.weight ? 'scaleY(0)' : 'scaleY(1)',
              zIndex: isWeighing.type != TT.weight ? 'auto' : theme.zIndex.modal + 1,
              transition: theme.transitions.create('transform', {
                easing: theme.transitions.easing.sharp,
                duration: isWeighing.type != TT.weight ? theme.transitions.duration.leavingScreen : theme.transitions.duration.enteringScreen,
              }),
            })}>
              <Slider
                sx={{ paddingLeft: 2, paddingRight: 1, height: '100%' }}
                slotProps={{
                  valueLabel: { style: { top: 22, right: 'unset', left: 32 } },
                } as any}
              ...
              {Object.values(TT).filter((tt) => this.state.advRule || tt != TT.or).map((tt) =>
                <FormControlLabel key={tt} value={tt}
                                  control={<Radio sx={theme => ({
                                    ...(this.props.tutorial == SDGT.edit2 && tt == TT.all && { borderWidth: 2, borderColor: theme.palette.secondary.main, borderStyle: 'solid' }),
                                  })}/>}
                                  label={en.get(tt)} />
              )}
            </RadioGroup>
                ...
                <List disablePadding>
                  {isEditing.rules.map((wg, i) =>
                    <ListItem key={i} disableGutters>
                      <ListItemIcon>
                        <IconButton
                          sx={{ padding: 0, fontSize: '1.125rem' }}
