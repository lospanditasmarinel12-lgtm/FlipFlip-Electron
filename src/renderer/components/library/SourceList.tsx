...
const BlacklistInputTextField = styled(TextField)({
  '& .MuiInputBase-input': {
    minWidth: 550,
    minHeight: 300,
    whiteSpace: 'nowrap',
...
class SourceList extends React.Component {
  ...
  onDelete(source: LibrarySource) {
    // Desktop: removing a source only unlinks it from the scene/library — it
    // must NEVER delete the user's own files on disk. (Mobile copies media into
    // app-owned storage, so it offers a separate "delete file" option.)
  ...
  onOpenSubtitleFile() {
    remote.dialog.showOpenDialog(remote.getCurrentWindow(), {filters: [{name:'All Files (*.*)', extensions: ['*']}, {name: 'Web Video Text Tracks (WebVTT)', extensions: ['vtt']}], properties: ['openFile']}).then(result => {
