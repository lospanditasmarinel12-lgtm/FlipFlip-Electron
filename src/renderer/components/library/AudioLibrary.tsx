...
import {extractMusicMetadata, getFilesRecursively, listChanged} from "../../data/utils";
...
class AudioLibrary extends React.Component {
      ...
      <Box sx={{ display: 'flex' }}>
        <AppBar position="absolute"
          ...
          <Toolbar sx={{ display: 'flex', alignItems: 'center', whiteSpace: 'nowrap', flexWrap: 'nowrap', minHeight: 64, height: (theme: Theme) => theme.mixins.toolbar.minHeight }}>
            ...
            <Box sx={{ flexBasis: '20%', justifyContent: 'flex-end', display: 'flex' }}>
              <Box sx={(theme) => ({
        ...
        <Drawer
          sx={(theme) => ({
            position: 'absolute',
            ...(this.props.tutorial == ALT.sidebar1 || this.props.tutorial == ALT.sidebar2 || this.state.drawerOpen) && {
              zIndex: theme.zIndex.modal + 1,
            },
          ...
          <Box sx={!open ? { display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 8px', minHeight: 64 } : undefined}>
            <Collapse in={!open}>
              <Box sx={(theme) => ({ backgroundColor: theme.palette.primary.main, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 8px', minHeight: 64 })} />
            </Collapse>
          </Box>
          ...
          <Box sx={(theme) => ({
            ...
            <Tooltip disableInteractive title={this.state.drawerOpen ? "" : "Manage Tags"}>
              <ListItemButton onClick={this.props.onManageTags.bind(this)} disabled={this.props.specialMode != null}>
                <ListItemIcon>
                  <LocalOfferIcon />
                </ListItemIcon>
            ...
            <Tooltip disableInteractive title={this.state.drawerOpen ? "" : "Add to Playlist"}>
              <ListItemButton onClick={this.onAddToPlaylist.bind(this)} disabled={this.props.specialMode != null}>
                <ListItemIcon>
                  <PlaylistAddIcon />
                </ListItemIcon>
            ...
            <Tooltip disableInteractive title={this.state.drawerOpen ? "" : "Batch Tag"}>
              <ListItemButton onClick={this.onBatchTag.bind(this)} disabled={this.props.specialMode != null}>
                <ListItemIcon>
            ...
            <Tooltip disableInteractive title={this.state.drawerOpen ? "" : "Batch Edit"}>
              <ListItemButton onClick={this.onBatchEdit.bind(this)} disabled={this.props.specialMode != null}>
                <ListItemIcon>
                  <EditIcon />
                </ListItemIcon>
          ...
          <Box sx={(theme) => ({
            ...
            <Tooltip disableInteractive title={"BPM Detection"}>
              <ListItemButton disabled={this.props.progressMode != null} onClick={this.props.onBatchDetectBPM.bind(this)}>
                <ListItemIcon>
        ...
        <Box component="main" sx={(theme) => ({ display: 'flex', flexGrow: 1, flexDirection: 'column', height: '100vh', backgroundColor: theme.palette.background.default })}>
          <Box sx={(theme) => ({ backgroundColor: theme.palette.primary.main, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 8px', minHeight: 64 })} />
          <Container maxWidth={false} sx={{ height: '100%', padding: 0, overflowY: 'auto' }}>
            ...
            {this.props.openTab === 0 && (
              <Typography component="div">
                <Box sx={{ display: 'flex', height: '100%' }}>
                  <Box sx={(theme) => ({ width: theme.spacing(7), minWidth: theme.spacing(7), [theme.breakpoints.up('sm')]: { width: theme.spacing(9), minWidth: theme.spacing(9) } })}/>
            ...
            {this.props.openTab === 1 && (
              <Typography
                ...
                <Box sx={{ display: 'flex', height: '100%' }}>
                  <Box sx={(theme) => ({ width: theme.spacing(7), minWidth: theme.spacing(7), [theme.breakpoints.up('sm')]: { width: theme.spacing(9), minWidth: theme.spacing(9) } })}/>
                  <Box p={2} sx={{ flexGrow: 1, position: 'relative', overflow: 'hidden' }}>
                    <AudioArtistList
        ...
        {this.props.specialMode && this.props.openTab == 3 && (
          <React.Fragment>
            <Tooltip disableInteractive title="Clear"  placement="top-end">
              <Fab
                sx={(theme) => ({ backgroundColor: theme.palette.secondary.light, margin: 0, top: 'auto', right: 180, bottom: 20, left: 'auto', position: 'fixed' })}
            ...
            <Tooltip disableInteractive title="Select All"  placement="top-end">
              <Fab
                sx={(theme) => ({ backgroundColor: theme.palette.secondary.dark, margin: 0, top: 'auto', right: 130, bottom: 20, left: 'auto', position: 'fixed' })}
            ...
            {this.props.specialMode == SP.batchTag && (
              <Tooltip disableInteractive title={"Batch Tag"}  placement="top-end">
                <Badge
                  slotProps={{
                    badge: {
                      sx: { top: 'auto', right: 30, bottom: 50, left: 'auto', position: 'fixed', zIndex: (theme: any) => theme.zIndex.fab + 1 },
                    },
                  }}
                  overlap="circular"
                  color="secondary"
                  ...
                  max={999}>
                  <Fab
                    sx={(theme) => ({ backgroundColor: theme.palette.primary.dark, margin: 0, top: 'auto', right: 20, bottom: 20, left: 'auto', position: 'fixed' })}
                    disabled={this.state.selected.length == 0}
                    onClick={this.onToggleBatchTagModal.bind(this)}
                    size="large">
                    <LocalOfferIcon sx={{ color: 'primary.contrastText' }} />
                  </Fab>
                </Badge>
              </Tooltip>
            )}
              ...
              <Tooltip disableInteractive title={"Import"}  placement="top-end">
                <Badge
                  ...
                  <Fab
                    ...
                    size="large">
                    <GetAppIcon sx={{ color: 'primary.contrastText' }} />
                  </Fab>
                </Badge>
              </Tooltip>
            )}
          ...
          <React.Fragment>
            {this.props.library.length > 0 && (
              <Tooltip disableInteractive title={this.state.filters.length == 0 ? "Delete All Sources" : playlist ? "Delete Playlist" : "Delete These Sources"}  placement="left">
                <Fab
                  sx={(theme) => ({ backgroundColor: theme.palette.error.main, margin: 0, top: 'auto', right: 130, bottom: 20, left: 'auto', position: 'fixed' })}
                  onClick={this.onRemoveAll.bind(this)}
                  size="small">
                  <DeleteSweepIcon sx={{ color: 'primary.contrastText' }} />
                </Fab>
              </Tooltip>
            )}
            <Dialog
            ...
            <Tooltip disableInteractive title={this.state.filters.length > 0 ? "" : "URL"}  placement="left">
              <Fab
                ...
                size="small">
                <HttpIcon sx={{ color: 'primary.contrastText' }} />
              </Fab>
            </Tooltip>
            {this.state.loadingSources && <CircularProgress size={60} color="secondary" sx={{ position: 'absolute', bottom: 18, right: 18, zIndex: 1 }} />}
            <Fab
              sx={(theme) => ({
        ...
        {this.props.openTab == 3 && (
          <React.Fragment>
            <Fab
              ...
              sx={(theme) => ({ backgroundColor: theme.palette.secondary.dark, margin: 0, top: 'auto', right: 80, bottom: 20, left: 'auto', position: 'fixed' })}
              aria-haspopup="true"
              aria-controls="sort-menu"
              aria-label="Sort Sources"
              onClick={this.onOpenSortMenu.bind(this)}
              size="medium">
              <SortIcon sx={{ color: 'primary.contrastText' }} />
            </Fab>
            <Menu
              id="sort-menu"
              elevation={1}
        ...
        {this.state.openMenu == MO.urlImport && (
          <Dialog
            slotProps={{ paper: { sx: { overflow: 'visible', width: '100%' } } }}
            ...
            <DialogActions>
              {this.state.loadingMetadata && <CircularProgress size={34} sx={{ position: 'absolute', right: 20 }} />}
              <Button
                sx={(theme) => ({
                  ...(this.state.error && {
  ...
  componentDidUpdate(props: any, state: any) {
    if (state.filters != this.state.filters || props.library != this.props.library || listChanged(props.library, this.props.library) || props.playlists != this.props.playlists) {
      this.setState({displaySources: this.getDisplaySources()});
    }
    ...
    switch (type) {
      ...
      case AF.audios:
        (async () => {
          ...
          if (e.shiftKey) {
            const adResult = await remote.dialog.showOpenDialog(remote.getCurrentWindow(),
            ...
            for (let path of adResult.filePaths) {
            ...
            const fileResult = await remote.dialog.showOpenDialog(remote.getCurrentWindow(),
...
function AudioLibraryWrapper(props: any) {
  const progress = useStore(useShallow((s: any) => ({
    progressCurrent: s.progressCurrent,
    progressTotal: s.progressTotal,
    progressTitle: s.progressTitle,
    progressMode: s.progressMode,
  })));
  return <AudioLibrary {...props} {...progress} />;
}
(AudioLibraryWrapper as any).displayName = "AudioLibraryWrapper";
export default AudioLibraryWrapper;