...
/**
 * True when two arrays differ in content (length or per-item identity key),
 * even if both are the same reference. Lets list components refresh their
 * displayed rows when items are added/removed in place.
 */
export function listChanged(a: Array<any>, b: Array<any>): boolean {
  if (a === b) return false;
  if (a == null || b == null) return a != b;
  if (a.length !== b.length) return true;
  for (let i = 0; i < a.length; i++) {
    if (a[i] === b[i]) continue;
    const aKey = a[i]?.id ?? a[i]?.url ?? a[i]?.name;
    const bKey = b[i]?.id ?? b[i]?.url ?? b[i]?.name;
...
export function generateThumbnailFile(cachePath: string, data: Buffer): string {
  ...
  if (!fs.existsSync(checksumThumbnailPath)) {
    // Recursive so the first audio import (before any image has been cached)
    // also creates the missing `ImageCache` parent. Otherwise a fresh install
    // throws ENOENT on `<saveDir>/imageCache/thumbs`.
    fs.mkdirSync(checksumThumbnailPath, { recursive: true });
  }
