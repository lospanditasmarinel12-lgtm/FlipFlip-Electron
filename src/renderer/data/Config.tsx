...
interface DisplaySettingsI {
  [key: string]: number | boolean | Array<string> | string;
  alwaysOnTop: boolean;
  showMenu: boolean;
  fullScreen: boolean;
...
interface GeneralSettingsI {
  [key: string]: number | boolean | string | Record<string, number>;
...
hapticTransport = 'auto' as string;
  hapticWSEndpoint = 'ws://127.0.0.1:12345' as string;
  hapticSystemAudio = false as boolean;
  hapticSystemDeviceId = '' as string;
  hapticMotorIntensities = {} as Record<string, number>;
}
...
export class LogSettings implements LogSettingsI {
  [key: string]: boolean;

  // All logging off by default to minimize console/perf overhead. Enable any
  // namespace from Settings → General Settings → Logging & Diagnostics.
  imagePlayer = false;
  memory = false;
  meta = false;
  audio = false;
...
export default class Config {
  ...
  constructor(init?: Partial<Config>) {
    ...
    for (let key of Object.keys(new LogSettings())) {
      if (this.logSettings[key] == null) {
        this.logSettings[key] = new LogSettings()[key];
      }
    }
