...
export function changeThemeColor(state: State, colorTheme: any, primary: boolean): Object {
  if (primary) {
    state.theme.palette.primary = colorTheme;
    const type = state.theme.palette.mode;
    if (type === "dark") {
      (state.theme.palette as any).background = {};
    } else if (type === "light") {
      (state.theme.palette as any).background = {default: colorTheme[50]};
    }
  } else {
    state.theme.palette.secondary = colorTheme;
  }
...
export function cleanBackups(config: Config) {
  ...
  if (config.generalSettings.autoCleanBackup) {
    ...
    for (let backup of backups) {
      ...
      if (isSameDay(backupDate, lastDay)) {
        if (isSameDay(backupDate, new Date()) && backupDate > lastDay) {
          keepDays[keepDays.length - 1] = backup;
        } else if (!isSameDay(backupDate, new Date()) && backupDate < lastDay) {
          keepDays[keepDays.length - 1] = backup;
        }
      ...
      if (isSameWeek(backupDate, lastWeek)) {
      ...
      if (isSameMonth(backupDate, lastMonth)) {
        if (backupDate < lastWeek) {
...
export function cacheImage(state: State, i: HTMLImageElement | HTMLVideoElement) {
  if (state.config.caching.enabled) {
    ...
    if (fileType != ST.local && i.src.startsWith("http")) {
      ...
      let cachedFolderSize: {size: number, timestamp: number} | null = null;
      if (maxSize == 0) {
        downloadImage();
      } else {
        const checkFolderSize = () => {
          const now = Date.now();
          if (cachedFolderSize && (now - cachedFolderSize.timestamp) < 60000) {
            const mbSize = cachedFolderSize.size;
            if (mbSize < maxSize) {
              downloadImage();
            }
            return;
          ...
          getFolderSize(cachePath, (err: string, size: number) => {
            ...
            cachedFolderSize = {size: mbSize, timestamp: Date.now()};
            if (mbSize < maxSize) {
              downloadImage();
            }
          });
...
export function cloneScene(state: State, scene: Scene): Object {
  ...
  const sceneCopy = { ...scene, id };
  return {
    scenes: [...state.scenes, sceneCopy],
...
export function saveScene(state: State, scene: Scene): Object {
  ...
  const sceneCopy = { ...scene, generatorWeights: null as any as any, id, openTab: 3 };
  return {
...
export function playScript(state: State, source: CaptionScript, sceneID: number, displayed: Array<CaptionScript>): Object {
  ...
  const tempScene = { ...state.scenes.find((s) => s.id == sceneID), id, libraryID: librarySource.id, scriptScene: true, textEnabled: true, scriptPlaylists: [{scripts: displayed, shuffle: false, repeat: RP.all}], scriptStartIndex: startIndex };
  return {
    scenes: state.scenes.concat([tempScene]),
    route: state.route.concat([new Route({kind: 'scene', value: tempScene.id}), new Route({kind: 'libraryplay', value: tempScene.id})]),
  };
...
export function updateScene(state: State, scene: Scene, fn: (scene: Scene) => void): Object {
  // Always produce a NEW scenes array reference so subscribed components (whose
  // componentDidUpdate checks rely on reference/array identity) re-sync their
  // derived state after a mutation.
...
export function updateAudioLibrary(state: State, fn: (audios: Array<Audio>) => void): Object {
  fn(state.audios);
  for (let url of state.audioSelected) {
    if (state.audios.find((s: Audio) => s.url == url) == null) {
...
export function updateScriptLibrary(state: State, fn: (scripts: Array<CaptionScript>) => void): Object {
  fn(state.scripts);
  for (let url of state.scriptSelected) {
    if (state.scripts.find((s: CaptionScript) => s.url == url) == null) {
...
export function updateLibrary(state: State, fn: (library: Array<LibrarySource>) => Array<LibrarySource> | void): Object {
  const replacement = fn(state.library);
  if (replacement) {
    state.library = replacement;
  }
  for (let url of state.librarySelected) {
    if (state.library.find((s: LibrarySource) => s.url == url) == null) {
...
export function updateTags(state: State, tags: Array<Tag>): Object {
  ...
  const tagIndexMap = new Map<number, number>();
  for (let i = 0; i < tags.length; i++) {
    tagMap.set(tags[i].id, tags[i]);
    tagIndexMap.set(tags[i].id, i);
  }
  const updateSourceTags = (sourceTags: Tag[]) => {
    sourceTags = sourceTags.filter((t: Tag) => tagMap.has(t.id));
    sourceTags = sourceTags.map((t: Tag) => {
    ...
    sourceTags = sourceTags.sort((a: Tag, b: Tag) => {
      const aIndex = tagIndexMap.get(a.id) ?? -1;
      const bIndex = tagIndexMap.get(b.id) ?? -1;
      return aIndex - bIndex;
    });
    return sourceTags;
  };
  for (let source of newLibrary) {
    source.tags = updateSourceTags(source.tags);
    if (source.clips) {
      for (let clip of source.clips) {
        if (clip.tags) {
          clip.tags = updateSourceTags(clip.tags);
        } else {
          clip.tags = [];
        }
      }
  ...
  switch (type) {
    ...
    case AF.url:
      if (!args || args.length != 1) {
        if (scene != null) {
          return updateScene(state, scene, (s) => {
            s.sources = addSources(s.sources, [""], state.library);
            handleArgs(s);
          })
        } else {
        ...
        if (scene != null) {
          return updateScene(state, scene, (s) => {
            s.sources = addSources(s.sources, args[0], state.library);
            handleArgs(s);
          })
        } else {
    ...
    case AF.list:
      ...
      if (scene != null) {
        return updateScene(state, scene, (s) => {
          s.sources = addSources(s.sources, newSources, state.library);
          handleArgs(s);
        })
      } else {
    ...
    case AF.directory:
      const dirSavedSetState = (state as any)._setState;
      remote.dialog.showOpenDialog(remote.getCurrentWindow(), {properties: ['openDirectory', 'multiSelections']}).then(dResult => {
        if (dResult.canceled || !dResult.filePaths.length) return;
        if (dirSavedSetState && scene != null) {
          dirSavedSetState((currentState: any) => {
            const newScenes = currentState.scenes.map((s: Scene) => {
              if (s.id === scene.id) {
                const newSources = [...s.sources];
                const mergedSources = addSources(newSources, dResult.filePaths, currentState.library);
                return { ...s, sources: mergedSources };
              }
              return s;
            });
            return { scenes: newScenes };
          });
        } else if (dirSavedSetState) {
          dirSavedSetState((currentState: any) => {
            const newLibrary = [...currentState.library];
            const mergedLibrary = addSources(newLibrary, dResult.filePaths, currentState.library);
            return { library: mergedLibrary };
          });
        }
      });
      return;
    ...
    case AF.videos:
      const vidSavedSetState = (state as any)._setState;
      remote.dialog.showOpenDialog(remote.getCurrentWindow(),
        {filters: [{name:'All Files (*.*)', extensions: ['*']}, {name: 'Video files', extensions: ['mp4', 'mkv', 'webm', 'ogv', 'mov']}, {name: 'Playlist files', extensions: ['asx', 'm3u8', 'pls', 'xspf']}], properties: ['openFile', 'multiSelections']}).then(vResult => {
        if (vResult.canceled || !vResult.filePaths.length) return;
        const filtered = vResult.filePaths.filter((r) => isVideo(r, true) || isVideoPlaylist(r, true));
        if (vidSavedSetState && scene != null) {
          vidSavedSetState((currentState: any) => {
            const newScenes = currentState.scenes.map((s: Scene) => {
              if (s.id === scene.id) {
                const newSources = [...s.sources];
                const mergedSources = addSources(newSources, filtered, currentState.library);
                return { ...s, sources: mergedSources };
              }
              return s;
            });
        ...
        } else if (vidSavedSetState) {
          vidSavedSetState((currentState: any) => {
            const newLibrary = [...currentState.library];
            const mergedLibrary = addSources(newLibrary, filtered, currentState.library);
            return { library: mergedLibrary };
          });
        }
      });
    ...
    case AF.videoDir:
      const vdSavedSetState = (state as any)._setState;
      remote.dialog.showOpenDialog(remote.getCurrentWindow(),
        {filters: [{name:'All Files (*.*)', extensions: ['*']}], properties: ['openDirectory', 'multiSelections']}).then(vdResult => {
        if (vdResult.canceled || !vdResult.filePaths.length) return;
        let rvResult = new Array<string>();
        for (let path of vdResult.filePaths) {
        ...
        if (vdSavedSetState && scene != null) {
          vdSavedSetState((currentState: any) => {
            const newScenes = currentState.scenes.map((s: Scene) => {
              if (s.id === scene.id) {
                const newSources = [...s.sources];
                const mergedSources = addSources(newSources, rvResult, currentState.library);
                return { ...s, sources: mergedSources };
              }
              return s;
            });
        ...
        } else if (vdSavedSetState) {
          vdSavedSetState((currentState: any) => {
            const newLibrary = [...currentState.library];
            const mergedLibrary = addSources(newLibrary, rvResult, currentState.library);
            return { library: mergedLibrary };
          });
        }
      });
    ...
    case GT.local:
      ...
      if (scene != null) {
        return updateScene(state, scene, (s) => {
          s.sources = addSources(s.sources, getImportURLs(args[0], rootDir), state.library);
          handleArgs(s);
        })
      } else {
    ...
    case GT.tumblr:
      ...
      if (scene != null) {
        return updateScene(state, scene, (s) => {
          s.sources = addSources(s.sources, getImportURLs(args[0]), state.library);
          handleArgs(s);
        })
      } else {
...
export function exportScene(state: State, scene: Scene): Object {
  ...
  const sceneCopy = { ...scene, generatorWeights: null as any as any, openTab: 3 };
            ...
            for (let c of r) {
              if (c.sceneID != -1) {
                ...
                if (cell && !scenesToExport.find((s) => s.id == c.sceneID)) {
                  const cellCopy = { ...cell, generatorWeights: null as any, openTab: 3 };
                    ...
                    for (let co of cell.overlays) {
                      if (!co.sceneID.toString().startsWith("999")) {
                        ...
                        if (overlay && !scenesToExport.find((s) => s.id == co.sceneID)) {
                          const overlayCopy = { ...overlay, generatorWeights: null as any, openTab: 3, overlays: [] as any };
  ...
  remote.dialog.showSaveDialog(remote.getCurrentWindow(),
    {filters: [{name: 'JSON Document', extensions: ['json']}], defaultPath: fileName}).then(result => {
      if (!result.canceled && result.filePath) {
        fs.writeFileSync(result.filePath, sceneExport);
      }
  });
  return {};
}
...
export function exportLibrary(state: State): Object {
  ...
  remote.dialog.showSaveDialog(remote.getCurrentWindow(),
    {filters: [{name: 'JSON Document', extensions: ['json']}], defaultPath: fileName}).then(result => {
      if (!result.canceled && result.filePath) {
        fs.writeFileSync(result.filePath, libraryExport);
      }
    });
  return {};
}
      ...
      if (existsSync(url)) {
        ...
        wretch(url)
          ...
          .blob(blob => {
            ...
            reader.onerror = () => bpmError(new Error('Failed to read blob'));
            reader.readAsArrayBuffer(blob);
          })
          .catch((e: Error) => bpmError(e));
      }
    } catch (e) {
      bpmError(e);
    }
...
function readImageDimensions(filePath: string): { width: number; height: number } | null {
  try {
    ...
    if (buf[0] === 0xFF && buf[1] === 0xD8) {
      ...
      fs.readSync(fd2, largeBuf, 0, 1024, 0);
      fs.closeSync(fd2);
      offset = 2;
      while (offset + 9 <= largeBuf.length) {
        if (largeBuf[offset] !== 0xFF) { offset++; continue; }
        const marker = largeBuf[offset + 1];
        if (marker >= 0xC0 && marker <= 0xC2) {
          return {
            height: largeBuf.readUInt16BE(offset + 5),
            width: largeBuf.readUInt16BE(offset + 7),
          };
        }
        offset += 2 + largeBuf.readUInt16BE(offset + 2);
      }
      return null;
    }
    ...
    if (buf.toString('ascii', 0, 4) === 'RIFF' && buf.toString('ascii', 8, 12) === 'WEBP') {
      ...
      if (vp8 === 'VP8X') {
        return {
          width: (buf.readUIntLE(24, 3) & 0x3FFF) + 1,
          height: ((buf.readUIntBE(24, 6) >> 2) & 0x3FFF) + 1,
        };
      }
      if (vp8 === 'VP8 ') {
        return {
...
export function updateVideoMetadata(getState: () => State, setState: Function) {
  ...
  const state = getState();
  const actionableLibrary = state.library.filter((ls) => {
    const st = getSourceType(ls.url);
    if (st == ST.video) {
      return ls.duration == null || ls.resolution == null || isNaN(ls.resolution) || ls.resolution <= 0 || ls.fileSize == null || isNaN(ls.fileSize);
    }
    if (st == ST.local && !ls.dirOfSources) {
      return ls.resolution == null || isNaN(ls.resolution) || ls.resolution <= 0 || ls.fileSize == null || isNaN(ls.fileSize);
    }
    return false;
  });
  ...
  const videoMetadataLoop = () => {
    ...
    if (state.progressMode == PR.cancel) {
      ...
      win.setProgressBar(-1);
      setState({
        systemSnack: "Media analysis has completed.",
        systemSnackSeverity: SS.success,
        progressMode: null,
        progressCurrent: 0,
        progressTotal: 0,
      ...
      const librarySource = state.library.find((s) => s.url == actionSource.url);
      if (librarySource) {
        const sourceType = getSourceType(librarySource.url);
        ...
        if (sourceType == ST.video) {
          ...
          try {
            const dims = readImageDimensions(librarySource.url);
            if (dims) {
              librarySource.resolution = Math.min(dims.width, dims.height);
            }
            try {
              librarySource.fileSize = fs.statSync(librarySource.url)?.size;
            } catch (e) {
          ...
          setState({progressCurrent: state.progressCurrent});
          win.setProgressBar(state.progressCurrent / state.progressTotal);
          setTimeout(videoMetadataLoop, 10);
        }
      } else {
        // Skip if removed from library during check
        state.progressCurrent = offset + 1;
