            ...
            scenes: (data.scenes.map((s: any) => {
              try {
                ...
                if (!Array.isArray(scene.sources)) {
                  // Defensive: an older/corrupted save can omit `sources` or
                  // store falsy entries. Never let one bad scene wipe the whole
                  // library via the self-heal archive path — coerce and log.
                  console.warn("[migrate] scene had non-array `sources`, coercing to []:", scene.id, scene.name);
                  scene.sources = [];
                }
                return scene;
              } catch (e) {
                console.error("[migrate] skipping unparsable scene:", e);
                return null;
              }
            }) as Array<any>).filter((s: any) => !!s),
            sceneGroups: data.sceneGroups ? data.sceneGroups.map((g: any) => new SceneGroup(g)) : [],
            grids: data.grids.map((g: any) => new SceneGrid(g)),
            audios: data.audios ? data.audios.map((a: any) => new Audio(a)) : [],
            scripts: data.scripts ? data.scripts.map((s: any) => new CaptionScript(s)) : [],
